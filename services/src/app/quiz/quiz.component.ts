...
export class QuizComponent {
    ...
    this._quizService._addQuiz(quizObj).subscribe(
      (data) => {
        ...
        if (this._quizMap.size > 0) {
          this._quizArray[this._quizArray.indexOf(quiz)] = Object.assign(
            {},
            this._quizMap.get(quiz.quizId)
          );
        }
  ...
  updateQuiz(quiz: Quiz, title: string) {
    this._quizService._getQuizByTitle(title).subscribe((data) => {
      this._quiz = data;
      this._quizService._updateQuiz(title, quiz).subscribe(
        (data) => {
