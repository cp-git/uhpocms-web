...
export class TeacherCourseService {
  ...
  constructor(private https: HttpClient, private cache: DataServiceCache) {
  ...
  getAllCourses(): Observable<any> {
    return this.https.get<any>(this.courseUrl + "?name=all");
  }

  //add a new course 
  addCourse(course: Course): Observable<any> {
    return this.https.post<any>(this.courseUrl, course);
  }

  //delete a course By ID
  deleteCourseByCourseId(courseId: number): Observable<any> {
    return this.https.delete<any>(this.courseUrl + '/courseId/' + courseId);
  }

  //update a course by ID
  updateCourseById(courseId: number, course: Course): Observable<any> {
    return this.https.put<any>(this.courseUrl + '/courseID/' + courseId, course);
  }

  //get a course by name
  getCourse(courseName: string): Observable<Course> {
    return this.https.get<Course>(this.courseUrl + '/' + courseName);
  }
  ...
  getCourseByCourseId(courseId: number): Observable<any> {
    ...
    return this.https.get<Course>(this.courseUrl + '/courseId/' + courseId).pipe(
      //   tap(
  ...
  getAllDeactivateCourses(): Observable<Course[]> {
    return this.https.get<Course[]>(`${this.courseUrl}?name=inactive`);
  }

  //activate a course
  activateCourseById(courseId: number): Observable<any> {
    return this.https.patch<any>(`${this.courseUrl}/activate/` + courseId, {});
  }
  // //get courses by department ID. 
  // getCourseByDepartmentId(deptid: number) {
  //   return this.https.get<any>(`${this.courseUrl}/deptId/` + deptid)
  // }
  ...
  getCourseByDepartmentId(deptid: number) {
    ...
    return this.https.get<any>(`${this.courseUrl}/deptId/` + deptid).pipe(
  ...
  getCourseByInstitutionId(id: string): Observable<any> {
    ...
    return this.https.get<any>(this.courseUrl + '/institutionId/' + id).pipe(
      //   tap(data => this.cache.setDataInCache(`${this.courseProgressUrl}/courseprog?id=all`, data))
      // );
  ...
  getCourseByDepartmentIdAndProfileId(department_id: number, profileId: number): Observable<any> {
    return this.https.get<any>(this.courseUrl + '/department/' + department_id + '/profile/' + profileId);
  }


  assignCourseToDepartment(coursedepartment: CourseDepartment): Observable<any> {
    return this.https.post<any>(`${this.courseDepartmentUrl}/department`, coursedepartment);
  }

  getCoursesDepartmentId(): Observable<any> {

    return this.https.get<any>(`${this.courseDepartmentUrl}/department?id=all`);
  }
